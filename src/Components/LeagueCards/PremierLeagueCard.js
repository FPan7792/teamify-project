...
const LeagueCards = ({ datas }) => {
  ...
  return (
    <section
      className="border-green border-solid border-2 bg-premierleague-logo m-20  "
      // prefetcher l'ensemble des équipes de la ligue au survol du composant
      onMouseEnter={async () => {
        await queryClient.prefetchQuery("PLTeams", fetchTeamsFromLeagues, {
          staleTime: 3000000,
        });
        await setAllTeams(queryClient.getQueryData("PLTeams"));
      }}
    >
      {/* LIGUE 1 */}
